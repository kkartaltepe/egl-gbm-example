...
//
// Adapted from https://registry.khronos.org/EGL/extensions/MESA/EGL_MESA_platform_gbm.txt
// to demonstrate nvidia issues.
...
static struct my_config get_config(struct my_display dpy) {
  ...
  if (!eglChooseConfig(dpy.egl, egl_config_attribs, configs, num_configs,
                       &num_configs)) {
    abort();
  }
  if (num_configs == 0) {
  ...
  for (int i = 0; i < num_configs; ++i) {
    ...
    if (!eglGetConfigAttrib(dpy.egl, configs[i], EGL_NATIVE_VISUAL_ID,
                            &gbm_format)) {
      abort();
    }
...
static struct my_window get_window(struct my_config config) {
  ...
  uint64_t mods = 0x0300000000CDB014;
  window.gbm = gbm_bo_create_with_modifiers(config.dpy.gbm, 256, 256, 0x34325258,
                                  &mods, 1);
  if (!window.gbm) {
    abort();
  }
...
#else
  window.egl =
      eglCreateWindowSurface(config.dpy.egl, config.egl, window.gbm, NULL);
#endif
